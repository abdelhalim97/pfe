          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">{{this.$parent.lg=='en'? 'Display amperages' :'Afficher les amperages'}}</h3>
                <div class="card-tools">
                    <button class="btn btn-success" @click="newModal" data-toggle="modal" data-target="#addNew">{{this.$parent.lg=='en'? 'Add new amperage' :'Ajouter un amperage'}}<i class="fas fa-user-plus fa-fw"></i></button>
                </div>
              </div>
              <!-- /.card-header -->
              <div class="card-body table-responsive p-0">
                ...
                <table class="table table-hover text-nowrap">
                  ...
                  <tbody>
                    <tr v-for="ampere in amperes.data" :key="ampere.id">
                        <td>{{ampere.id}}</td>
                        <td><img v-bind:src="'/images/emperages/' + ampere.image" ></td>
                        <td>{{ampere.article}}</td>
                        <td>{{ampere.description}}</td>
                        <td>{{ampere.name}}</td>
                        <td>{{ampere.created_at |myDate}}</td>
                        <td>{{ampere.updated_at|myDate}}</td>
                      <td><a href="#" @click="editModal(ampere)"><i class="fa fa-edit"></i></a> / <a href="#" @click="deleteAmpere(ampere.id)"><i class="fa fa-trash red"></i></a></td>
                    </tr>
                  </tbody>
                </table>
                <div class="card-footer">
                  <pagination style="margin-bottom: 0px;"
 :data="amperes" @pagination-change-page="getResults"></pagination>
              </div>
              </div>
              <!-- /.card-body -->
            </div>
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" v-show="!editmode" id="addNewLabel">{{this.$parent.lg=='en'? 'Add new amperage' :'Ajouter un amperage'}}</h5>
        <h5 class="modal-title" v-show="editmode" id="addNewLabel">{{this.$parent.lg=='en'? 'Update amperage\'s information' :'Modifier les informations d\'amperage'}}</h5>
...
<script>
    export default {
        ...
        methods:{
            getResults(page = 1) {
                        axios.get('api/ampere?page=' + page)
                            .then(response => {
                                this.amperes = response.data;
  ...
  text: 'Some fields already has been taken',
            ...
            updateImage(){
            this.$Progress.start();
                this.form.put('/api/ampere/'+this.form.id)
              .then(()=>{
                $('#addNew').modal('hide')
                swal.fire(
                                        'Done!',
  ...
  text: 'Some fields already has been taken',
            ...
            loadAmperes(){
                    axios.get("/api/ampere").then(({ data }) => (this.amperes = data));
            },
            deleteAmpere(id){
                swal.fire({
                    title: 'Are you sure?',
                    ...
                    confirmButtonText: 'Yes, delete it!'
                        ...
                        // Send request to the server
                         if (result.value) {
                                this.form.delete('/api/ampere/'+id).then(()=>{
                                        swal.fire(
                                        'Deleted!',
                                        'The amperage has been deleted.',
                                        'success'
        ...
        created() {
            Fire.$on('searching',()=>{
                ...
                axios.get('api/findAmpere?q=' + query)
                .then((data)=>{
                    this.amperes=data.data
                })
                .catch(()=>{
            ...
            this.loadAmperes();
            Fire.$on('AfterCreated',()=>{this.loadAmperes();});
}
    }
</script>
